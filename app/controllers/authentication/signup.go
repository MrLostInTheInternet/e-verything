...
func SignupPOST(c echo.Context) (err error) {
	...
	if found.Error == nil { // No error means a record was found
		errorMessages["Email"] = "You already have an account with this email"
		return c.Render(http.StatusOK, "signup", echo.Map{
			"Errors": errorMessages,
			"Date":   time.Now().Year(),
		})
...
func validatePassword(password string, errorMessages map[string]string) {
	if len(password) < 8 {
	...
	if !hasUpper {
		errorMessages["Password"] = "Password must have at least one uppercase letter"
	}
	if !hasLower {
		errorMessages["Password"] = "Password must have at least one lowercase letter"
	}
	if !hasNumber {
		errorMessages["Password"] = "Password must have at least one number"
	}
	if !hasSpecial {
		errorMessages["Password"] = "Password must have at least one special character"
	}
}
